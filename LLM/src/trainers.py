...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str,
        ...
        #Add random data specific configs
        if 'dataset' in self.config.keys():
            
            print('dataset kwargs', self.config.dataset)
            
            data_iterator_kwargs.update(self.config.dataset)
    ...
    def get_batch_samples(self, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        ...
        if self.config.loss.name in self.preference_loss_names:
        ...
        if self.config.loss.name in self.preference_loss_names:
            ...
            reference_output_decoded = list()
            reference_output = list()

        return policy_output_decoded, policy_output, reference_output_decoded, reference_output
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if self.config.loss.name in self.preference_loss_names:
            ...
            #Add data specifically to the necessary loss configs:
            if self.config.loss.name in REQUIRE_TIMESTEP:
                timestep = batch['timestep']
            else: timestep = None
                
            losses, chosen_rewards, rejected_rewards = preference_loss(
                policy_chosen_logps, policy_rejected_logps, reference_chosen_logps, reference_rejected_logps,
                self.config.loss, timestep=timestep)
            ...
            #Calculate the reject logps only on the preference dataset:
            policy_rejected_logps = all_gather_if_needed(policy_rejected_logps.detach(), self.rank, self.world_size)
            metrics[f'logps_{train_test}/rejected'] = policy_rejected_logps.cpu().numpy().tolist()

        elif self.config.loss.name in self.sft_loss_names:

            #TODO: Calculate the rejected log probs for logging purposes -> use no grad
            # with torch.no_grad():
            #     policy_rejected_logits = self.policy(batch['rejected_input_ids'], 
            #             attention_mask=batch['rejected_attention_mask']).logits.to(torch.float32)
            #     policy_rejected_logps = _get_batch_logps(policy_rejected_logits, 
            #             batch['rejected_labels'], average_log_prob=False)
            ...
            elif self.config.loss.name  == 'ns_sft':
                
                assert 'timestep' in batch.keys(), 'ns_sft requires dataset with timestep key'
                
                exponents = self.config.loss.current_time - batch['timestep'] - 1
                gammas = self.config.loss.gamma ** exponents
                                
                losses = - policy_chosen_logps * gammas
            
            else:
                raise NotImplementedError(f'SFT loss: {loss_config.name} not implemented')
    ...
    def evaluation(self, batch):
        """
        Evaluate the performance of the model at every eval_every example counter step

        Parameters
        ----------
        batch : dict
            dictionary containing the encoded training inputs of the model
        ...
        if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
            ...
            if self.config.sample_during_eval:
                ...
                for eval_batch in (tqdm.tqdm(sample_batches, desc='Generating samples...') if self.rank == 0 else sample_batches):
                    local_eval_batch = slice_and_move_batch_for_device(eval_batch, self.rank, self.world_size, self.rank)
                    policy_samples, policy_sample_input_ids,\
                        reference_samples, _ = self.get_batch_samples(local_eval_batch)
                    
                    policy_sample_attn_masks, policy_sample_labels = process_samples(local_eval_batch['prompt_input_ids'],
                                                                         local_eval_batch['prompt_attention_mask'],
                                                                         policy_sample_input_ids)
                                        
                    #calculate probability of the samples under the policy and reference models:
                    with torch.no_grad():
                                                
                        policy_logits = self.policy(policy_sample_input_ids, attention_mask=policy_sample_attn_masks).logits.to(torch.float32)
                        policy_logps = _get_batch_logps(policy_logits, policy_sample_labels, average_log_prob=False)
                        
                        if self.config.loss.name in self.preference_loss_names:
                        
                            reference_logits = self.reference_model(policy_sample_input_ids, attention_mask=policy_sample_attn_masks).logits.to(torch.float32)
                            reference_logps = _get_batch_logps(reference_logits, policy_sample_labels, average_log_prob=False)

                            #Calculate the eval_KL divergence:
                            kl_div.extend(policy_logps - reference_logps)
            ...
            if self.config.sample_during_eval and self.config.print_sample_during_eval:
            ...
            if self.config.wandb.enabled and self.rank == 0:
                ...
                if self.config.sample_during_eval:
                    ...
                    if self.config.loss.name in self.preference_loss_names:
                        ...
                        empirical_kl_div = torch.concat(kl_div).mean().cpu().numpy()
                        wandb.log({'empirical_kl_div_eval': empirical_kl_div.cpu().numpy()}, step=self.example_counter)
    ...
    def save(self, output_dir: Optional[str] = None, metrics: Optional[Dict] = None):
        ...
        if self.scheduler is not None:  # MetaOptimize has no LR scheduler
